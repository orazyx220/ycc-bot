...
/**
 * Répond aux suggestions d'autocomplétion de l'option "recherche" :
 * propose les cartes dont le nom/ID contient le texte tapé (max 25).
 * `onlyInStock` limite aux cartes encore disponibles (pour la boutique).
 */
async function respondCardAutocomplete(interaction, onlyInStock = false) {
    ...
    const cards = await Card_1.Card.find(filter).limit(200);
    const results = cards
        .filter((c) => c.name.toLowerCase().includes(focused) || c.cardId.toLowerCase().includes(focused))
        .slice(0, 25)
        .map((c) => ({
        name: `${c.name} (${c.cardId})`.slice(0, 100),
