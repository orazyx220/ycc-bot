...
const DETAILS = {
    ...
    daily: {
        ...
        short: 'Réclame tes 550 Yumz du jour (1×/24 h).',
        summary: 'Donne **550 Yumz**, une fois toutes les 24 h. Un compteur t’empêche de le réclamer deux fois.',
        params: [],
        examples: ['/daily'],
        notes: 'Si tu l’as déjà fait, le bot te dit dans combien de temps tu pourras revenir.',
    },
    solde: {
...
function buildDetailEmbed(d) {
    const embed = new discord_js_1.EmbedBuilder()
        .setColor(d.admin ? 0xe67e22 : 0x5865f2)
        .setTitle(`📖 ${d.name}${d.admin ? '   🛡️ Admin' : ''}`)
        .setDescription(d.summary)
        .addFields({ name: 'Paramètres', value: formatParams(d.params) }, { name: 'Exemple(s)', value: d.examples.map((e) => `\`${e}\``).join('\n') });
    if (d.notes)
        embed.addFields({ name: 'Bon à savoir', value: d.notes });
    return embed;
}
/**
