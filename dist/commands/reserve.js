...
exports.reserve = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .setRequired(true)
        .addChoices({ name: 'Achat (coûte des Yumz)', value: 'buy' }, { name: 'Cadeau (gratuit)', value: 'gift' })))
        .addSubcommand((s) => s
        .setName('remove')
    ...
    async execute(interaction) {
        ...
        if (sub === 'list') {
            const cards = await Card_1.Card.find({ autoDrop: true });
            if (cards.length === 0) {
                await interaction.reply({
            ...
            const lines = cards.map((c) => `\`${c.cardId}\` — **${c.name}** · ${c.dropMode === 'gift' ? '🎁 cadeau' : '🛒 achat'} · reste ${c.remainingSupply}/${c.maxSupply}`);
            await interaction.reply({
